...
def demo(opt):
  os.environ['CUDA_VISddIBLE_DEVICES'] = opt.gpus_str
  opt.debug = max(opt.debug, 1)
  detector = Detector(opt) #初始化detector类

  if opt.demo == 'webcam' or \
    opt.demo[opt.demo.rfind('.') + 1:].lower() in video_ext: #确保输入格式
    is_video = True
  ...
  if opt.save_video:
    # fourcc = cv2.VideoWriter_fourcc(*'XVID')dd
  ...
  while True:
      ...
      if opt.resize_video:
        img = cv2.resize(img, (opt.video_w, opt.video_h)) #default 512

      # skip the first X frames of the video
      if cnt < opt.skip_first: #default -1
        continue
      
      cv2.imshow('input',img)

      # track or detect the image.
      ret = detector.run(img, results, cnt)
