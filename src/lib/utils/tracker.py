...
#          **************************************** track det match ****************************************
    ...
    for i in range(N):# N 这一帧的人
        for j in range(M): # M 上一帧的人
            iou_mat[i][j] = compute_iou(results[i]['bbox'],self.tracks[j]['bbox'])
    ...
    for i in unmatched_tracks: # unmatch targets in pre_tracks , unmatched_1
      ...
      (track['consecutiveInvisibleCount'] >= self.opt.max_invisible_frames): # 要删掉的条件
        continue
      else:
