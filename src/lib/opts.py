...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--test_dataset', default='',
                             help='coco | kitti | coco_hp | pascal | posetrack')
    ...
    self.parser.add_argument('--max_ages', type=int, default= 8)
    self.parser.add_argument('--max_invisible_frames',type=int,default=10)
    self.parser.add_argument('--min_iou',type=float,default=0.5)
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    opt.num_classes = dataset.num_categories if opt.num_classes < 0 else opt.num_classes #classes：person
  ...
  def init(self, args=''):
    ...
    default_dataset_info = {
      'ctdet': 'coco', 'multi_pose': 'posetrack', 'ddd': 'nuscenes',
      'tracking,ctdet': 'coco', 'tracking,multi_pose': 'posetrack',
