...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, resdet, cur_cnt, meta={}):
    ...
    for scale in self.opt.test_scales:
      ...
      if self.opt.tracking:
        ...
        if self.pre_images is None:
          ...
          self.tracker.init_track(
            meta['pre_dets'] if 'pre_dets' in meta else [], resdet, cur_cnt)
        if self.opt.pre_hm:
      ...
      detections.append(result) #还没有加上tracking-id
      if self.opt.debug >= 2:
        self.debug(
    ...
    if self.opt.tracking:
      ...
      results = self.tracker.step(results, resdet, cur_cnt, public_det)
      self.pre_images = images
...
#the network
  def process(self, images, pre_images=None, pre_hms=None,
    ...
    with torch.no_grad():
      torch.cuda.synchronize()
      outputmp, mu = self.model(images, pre_images, pre_hms)
      output = outputmp[-1]
      ...
      torch.cuda.synchronize()
      for k in dets:
        dets[k] = dets[k].detach().cpu().numpy()#转换为numpy
    if return_time:
  ...
  #chexiaotong keyframe model begin
  # def _keymatch(self,meta,output):
  #   keylen = 5;
  #   if len(self.keyframe)>keylen: #关键帧列表长度为5帧
  #     self.keyframe = self.keyframe[keylen:]
  #   pre_dets = output['pre_dets'] #前一帧的内容
  #   dets = output['dets'] #本帧内容
  #   for id in pre_dets['inds']:
  #     if id not in dets['inds']:
  #       self.keyframe.append(pre_dets)
  #       self.lost_inds.append(id)
  #   for id in dets['inds'] and id not in pre_dets['inds']:#new_ids
  #     for keyframe in self.keyframe:
  #       for lost_id in self.lost_inds:
  #         if lost_id in keyframe['inds']:
  #           matched_indices = Tracker.greedy_assignment(dets['inds'][id],keyframe['inds'][lost_id])
  #           if matched_indices[id,lost_id]:
  #             dets['inds'][id]['tracking_id'] = keyframe['inds'][lost_id]['tracking_id']
  #chexiaotong keyframe model end
