...
class ModleWithLoss(torch.nn.Module):
  ...
  def forward(self, batch):
    ...
    outputs, mu = self.model(batch['image'], pre_img, pre_hm)
    loss, loss_stats = self.loss(outputs, batch)
    # writer.add_scalar('hist',loss.item(),iteration)
    return outputs[-1], loss, loss_stats, mu
...
class Trainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      output, loss, loss_stats, mu = model_with_loss(batch)

      with torch.no_grad():
        mu = mu.mean(dim = 0, keepdim = True)
        momentum = 0.9
        self.model.emau.mu *= momentum
        self.model.emau.mu += mu *(1 - momentum)
