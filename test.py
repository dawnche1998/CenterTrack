...
class PrefetchDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if 'frame_id' in img_info and int(str(img_info['frame_id'])[-4:])== 0: #for posetrack #chexiaotong
    # if 'frame_id' in img_info and img_info['frame_id'] == 1: #chexiaotong
...
def prefetch_test(opt):
  ...
  for ind, (img_id, pre_processed_images) in enumerate(data_loader):
    ...
    if opt.tracking and ('is_first_frame' in pre_processed_images):
      # if '{}'.format(int(img_id.numpy().astype(np.int32)[0])) in load_results: #chexiaotong astype(np.int32)
      if '{}'.format(int(img_id.numpy()[0])) in load_results:
        pre_processed_images['meta']['pre_dets'] = load_results['{}'.format(int(img_id.numpy()[0]))]
        # load_results['{}'.format(int(img_id.numpy().astype(np.int32)[0]))]
      else:
        print()
        # print('No pre_dets for', int(img_id.numpy().astype(np.int32)[0]),
        print('No pre_dets for', int(img_id.numpy()[0]),
          '. Use empty initialization.')
        pre_processed_images['meta']['pre_dets'] = []
      detector.reset_tracking()
      print('Start tracking video', str(pre_processed_images['video_id'][0]))
    if opt.public_det:
      # if '{}'.format(int(img_id.numpy().astype(np.int32)[0])) in load_results:
      if '{}'.format(int(img_id.numpy()[0])) in load_results:
        pre_processed_images['meta']['cur_dets'] = \
          load_results['{}'.format(int(img_id.numpy()[0]))]
        # load_results['{}'.format(int(img_id.numpy().astype(np.int32)[0]))]
      else:
        # print('No cur_dets for', int(img_id.numpy().astype(np.int32)[0]))
        print('No cur_dets for', int(img_id.numpy()[0]))
        pre_processed_images['meta']['cur_dets'] = []

    
    ret = detector.run(pre_processed_images,results,ind)
    # results[int(img_id.numpy().astype(np.int32)[0])] = ret['results']
    results[int(img_id.numpy()[0])] = ret['results']
