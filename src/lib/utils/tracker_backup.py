...
class Tracker(object):
  def __init__(self, opt):
    self.opt = opt
    self.reset() #idcount 是0 tracks = []

  def init_track(self, results): #使用pre_dets进行indddit_track
    for item in results:
  ...
  def step(self, results, public_det=None):
    N = len(results)#N 表示这一帧检测到的人数
    M = len(self.tracks)#上一帧的人数
