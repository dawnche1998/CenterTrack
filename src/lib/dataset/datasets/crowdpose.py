...
class CrowdPose(GenericDataset):
    num_categories = 1
    class_name = ['']
    num_joints = 17
    ...
    def __init__(self, opt, split):
        data_dir = os.path.join(opt.data_dir, 'crowdpose')
        img_dir = os.path.join(data_dir,'CrowdPose_images','images')
        ann_path=os.path.join(data_dir,'annotations','crowdpose_{}.json').format(split)
        self.images = None
        # load image list and coco
        # load image list and coco
        super(CrowdPose, self).__init__(opt, split, ann_path, img_dir)
        if split == 'train':
    ...
    def save_results(self, results, save_dir):
        json.dump(self.convert_eval_format(results),
                  open('{}/results_crowdpose.json'.format(save_dir), 'w'))  # 在保存路径的results_cocohp.json文件写入结果
    ...
    def run_eval(self, results, save_dir):
        ...
        coco_dets = self.coco.loadRes('{}/results_crowdpose.json'.format(save_dir))
